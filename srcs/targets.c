...
{
	if (((int)render_data->targets[i].pos_x == (int)next_x
			&& (int)render_data->targets[i].pos_y
			== (int)render_data->player.pos_y)
		|| ((int)render_data->player.pos_x == (int)render_data->targets[i].pos_x
			&& (int)next_y == (int)render_data->targets[i].pos_y))
	{
		render_data->targets[i].active = 0;
