...
/*   Updated: 2024/07/16 13:43:26 by hwiemann         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			mlx_texture_t *tex)
{
	...
	{
		tex_y = (int)render_data->raycast.tex_pos;
		render_data->raycast.tex_pos += render_data->raycast.tex_step_size;
		if (tex)
		{
			uint32_t	color_tex = ((uint32_t*)tex->pixels)[tex->width * tex_y
				+ render_data->raycast.tex_x];
			...
			if (r != 0 || g != 0 || b != 0)
			{
				image->pixels[(y * image->width + image->width - 1 - x)
					* 4 + 0] = a;
				image->pixels[(y * image->width + image->width - 1 - x)
					* 4 + 1] = g;
				image->pixels[(y * image->width + image->width - 1 - x)
